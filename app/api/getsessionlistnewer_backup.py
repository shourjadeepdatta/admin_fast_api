...
def get_session_list_new(client_name, request_json):
  ...
  if True:
    ...
    res_items = postgres.call_function(db_start_time,db_end_time)

    merged_df = abhishek.get_session_user_data_from_dynamo(res_items)
    merged_df["user_id"] = merged_df["user_id"].apply(lambda x:eval(x))
    res_items = merged_df[["end_time_x","user_id","session_status","auditor_name","disposition_punched","CUSTNAME","ekyc_req_time_x","manual_agent","queue","audit_result","audit_lock","auditor_fdbk"]]#.to_dict(orient="records")
    #res_items = merged_df
    ...
    res_items["ekyc_status_calc"] = res_items["ekyc_req_time"].apply(lambda x: "KYC_PENDING" if divmod(time.time() - float(x), 60)[0] > ekyc_expiry_timer else "VCIP_PENDING")
    ...
    res_items['session_status'] = res_items.apply(lambda row: row['ekyc_status_calc'] if row['session_status'] in ['VCIP_SESSION_INVALID', 'user_abandoned'] else row['session_status'],axis=1)
    ...
    return jsonify({"data":res_items,"count":len(res_items),"agents_dict":agents_df.to_dict(orient="records")}),200
