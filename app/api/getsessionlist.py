...
def update_status_of_abandoned_and_invalid(row):
    ekyc_expiry_time_period = {"BFL": 4320, "RBL": 4310,"Retailasset":4310,"Digiremit":259200}

    if row["session_status"] in ["VCIP_SESSION_INVALID", "user_abandoned"] and row["ekyc_req_time"]:
        if (divmod(time.time() - float(row['ekyc_req_time']), 60)[0] <= ekyc_expiry_time_period[row['client_name']]):
            return "VCIP_PENDING"
        else:
            return "KYC_PENDING"
    return "KYC_PENDING"
...
def get_session_list(client_name, request_json):
    try:
        ...
        for session in one_d_session_list:
            ...
            try:
                session_extras["user_info"].pop("photo")
            except:
                pass
            try:
                session_extras["user_info"].pop("aadhaarImage") 
            except:
                pass
    ...
    if temp_df.shape[0] > 0:
        ...
        temp_df["ekyc_status_calc"] = temp_df["ekyc_req_time"].apply(lambda x: "KYC_PENDING" if (x == "" or divmod(time.time() - float(x), 60)[0] > ekyc_expiry_timer) else "VCIP_PENDING")
        ...
        if session_type and session_type in ("unique_user"):
            ...
            temp_df["session_status"] = temp_df.apply(update_status_of_abandoned_and_invalid, axis=1)
        ...
        temp_df['case_push_time'] = temp_df['user_id'].apply(get_case_push_time)
        ...
        temp_df = temp_df.sort_values(by=["user_id", "start_time"], ascending=[True, False]).reset_index(drop=True)

        temp_df["rbl_session_status"] = temp_df["session_status"]

        latest_statuses = temp_df.drop_duplicates(subset=["user_id"], keep="first")

        for index, row in latest_statuses.iterrows():
            user_id = row["user_id"]
            latest_status = row["session_status"]
            substitute_statuses = ["VCIP_SESSION_INVALID", "user_abandoned", "VCIP_RESCHEDULED"]
            if latest_status in substitute_statuses:
                temp_df.loc[temp_df["user_id"] == user_id, "rbl_session_status"] = temp_df.loc[temp_df["user_id"] == user_id, "ekyc_status_calc"]

        temp_df["session_status"] = temp_df["rbl_session_status"]

        try:
            temp_df.drop("get_first_session_detail_out", axis=1, inplace=True)
            temp_df.drop("extras", axis=1, inplace=True)
            temp_df.drop("session_extras", axis=1, inplace=True)
        except Exception as e:
            pass
